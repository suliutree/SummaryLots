...
#### vs2010中各类文件的作用
    ...
    工程配置属性中（附加包含目录）路径开始的地方是工程文件所在的地方
...
#### Debug情况下不出问题，Release出问题

    这种情况一般和变量没有初始化有关。可以从这方面入手检查一下变量的赋值情况。

<br>

#### 渲染流程中关于Pass的概念

    实际工业中使用的渲染方法会比经典的渲染管线要复杂的多，例如它可能需要分多次渲染（每次称为一个Pass），每次生成最终结果的部分内容，最后进行合成，例如对动态和静态物体的分开处理，对阴影的处理，对光照的处理，以及最后对全屏 Ambient Occlusion 的处理等。
